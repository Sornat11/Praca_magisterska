...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c|}
    \hline
    \textbf{Parametr statystyczny}        & \textbf{Wartość dla zbioru MovieLens 100K (\texttt{ml-100k})} \\ \hline
...
\begin{enumerate}
  \item W pierwszej kolejności, dla każdego z analizowanych elementów, system precyzyjnie ustala sąsiedztwo. Proces ten rozpoczyna się od wyliczenia funkcji podobieństwa $s(i, j)$ pomiędzy dowolnymi dwoma filmami $i$ oraz $j$ z katalogu $\mathcal{I}$. Ze względu na to, że badana w niniejszej pracy przestrzeń danych opiera się na interakcjach dorozumianych (\foreignlanguage{english}{\textit{implicit feedback}}), klasyczne miary uwzględniające średnie wariancje ocen (np. korelacja Pearsona) nie znajdują tu matematycznego zastosowania. Standardową techniką służącą do pomiaru bliskości w środowisku binarnym jest odległość kosinusowa (ang. \foreignlanguage{english}{\textit{Cosine Similarity}}), operująca w przestrzeni wektorowej. Stopień powiązania dwóch filmów wyznacza się za jej pomocą ze wzoru \footcite[16]{balchanowskiEwolucyjnaAgregacjaRang2023}:
        ...
        Gdzie:
        \begin{itemize}
          \item $\mathbf{y}_i, \mathbf{y}_j$ -- wektory binarne reprezentujące historię interakcji wszystkich użytkowników dla filmów $i$ oraz $j$
...
\begin{figure}[htbp]
  ...
  \caption{Architektura hybrydowego modelu Neural Matrix Factorization (NeuMF). Opracowanie własne na podstawie \footcite{heNeuralCollaborativeFilteringKwiecien032017}.}
...
\begin{figure}[htbp]
    ...
    \caption{Architektura algorytmu LightGCN z uwzględnieniem propagacji warstwowej. Opracowanie własne na podstawie \footcite{heLightGCNSimplifyingPowering2020}.}
